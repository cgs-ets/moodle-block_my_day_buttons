...
(function(self) {
    ...
    self.navigate = function(direction) {
        ...
        var args = {
            timetableuser: self.CONTENT_OTHERDATA.timetable.username,
            nav: direction,
            date: self.CONTENT_OTHERDATA.timetable.date,
            instanceid: self.CONTENT_OTHERDATA.timetable.instanceid
        }
        console.log('block_my_day_buttons/mobile/js/timetable.js: Navigate timetable: ' + JSON.stringify(args));
        self.CoreSitesProvider.getCurrentSite().read('block_my_day_buttons_get_timetable_data_for_date', args).then(function(timetabledata) {
