...
namespace block_my_day_buttons\output;
...
class mobile {
    ...
    public static function timetable_view(array $args) {
        ...
        $params = [$context->id, 'course-view-' . $course->format, '', $context->id, 'my_day_buttons'];
        $blockinstanceid = $DB->get_field_sql($sql, $params, IGNORE_MULTIPLE);

        $OUTPUT = $PAGE->get_renderer('core');
        $data = myday_init_timetable($blockinstanceid);
        $html = $OUTPUT->render_from_template('block_my_day_buttons/mobile', array());
        return [
            'templates' => [
                [
                    'id' => 'timetable',
            ...
            'javascript' => file_get_contents($CFG->dirroot . '/blocks/my_day_buttons/mobile/js/timetable.js'),
            'otherdata' => [
